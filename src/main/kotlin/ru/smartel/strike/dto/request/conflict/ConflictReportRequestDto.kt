...
data class ConflictReportRequestDto(
    @field:ApiParam(value = "Начальная дата отчета", defaultValue = "0")
    @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
    var from: Long = 0,
    @field:ApiParam(value = "Конечная дата отчета", defaultValue = "сегодня")
    @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
    var to: Long = LocalDateTime.now().toEpochSecond(ZoneOffset.UTC),
    @field:ApiParam(value = "Идентификаторы стран. В отчет попадут конфликты, " +
